...
class Program:
    def __init__(self,name=None,extension=None,compiledInto=None,tag=None,devClass=None,logicPath=None,type=None,moduleRequired=None):
        self.Name = name
...
def Compiled_dll(program,sourcePath,compiledPath):
    if (program.Extension == "c"):
        ...
        if (os.path.exists(compiledPath + "\\" + program.Name + "\\" + program.Name + ".cp37-win_amd64" + ".pyd") == False):#TODO:".cp37-win_amd64" must be depended on developers' devices
            ...
            shutil.copyfile(compiledPath + "\\" + program.Name + "\\" + program.Name + ".cp37-win_amd64" + ".pyd",compiledPath + "\\" + program.Name + ".pyd")#TODO:".cp37-win_amd64" must be depended on developers' devices
            os.remove(compiledPath + "\\" + program.Name)
            return True
    else:
        pass
...
#Process operate
def LogoPrinter():
    print(" __      __________________________      __            .__       .___ \n /  \    /  \__    ___/\_   _____/  \    /  \___________|  |    __| _/ \n \   \/\/   / |    |    |    __) \   \/\/   /  _ \_  __ \  |   / __ |  \n  \        /  |    |    |     \   \        (  <_> )  | \/  |__/ /_/ |  \n   \__/\  /   |____|    \___  /    \__/\  / \____/|__|  |____/\____ |  \n        \/                  \/          \/                         \/ ")
...
def CMDHelper():
    ...
    print(CMDHelper_MsgFormat("program add <name> <extension> <compiledInto> <tag> <class> <logicPath> <type> <moduleRequire>") + "Insert program config.")
    ...
    if (cmd == ""):
        ...
        if(cmd.split()[1] == "add"):
            if(cmd.split()[2] == "packList"):
                ...
                exec("{} = Program(\"{}\",\"{}\",\"{}\")".format(cmd.split()[2],cmd.split()[2],cmd.split()[3],cmd.split()[4]))
                if(cmd.split()[5]):
